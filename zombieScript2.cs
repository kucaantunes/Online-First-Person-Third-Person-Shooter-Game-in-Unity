  ...
  void OnTriggerEnter (Collider col)
  {
   ...
   // agent.destination = gameObject.transform.position;
    ...
    //destroy this zombie in six seconds.
	 ...
	 GameObject story = Instantiate(Resources.Load("storyvr", typeof(GameObject))) as GameObject;
	
    //Get the bullet's rigid body component and set its position and rotation equal to that of the spawnPoint
  //  Rigidbody rb = bullet.GetComponent<Rigidbody>();
  //  story.transform.rotation = spawnPoint.transform.rotation;
 //   story.transform.position = spawnPoint.transform.position;
